...
const createWindow = (): void => {
  ...
  board.on("ready", () => {
    ...
    button.on("press", () => {
      ...
      mainWindow.webContents.send("toggle-scene");
    });
    ...
    potentiometer.on("change", function () {
      ...
      mainWindow.webContents.send(
        "update-angle",
        (this.value / 1300.0) * 2.0 - 1.0
      );
    });

    let potentiometerTwo = new five.Sensor({
      pin: "A1",
      frequency: 250,
      threshold: 5,
    });
    potentiometerTwo.on("change", function () {
      console.log(this.value / 1020.0);
      mainWindow.webContents.send("scale-shape", this.value / 1020.0);
    });
  });

  setInterval(() => {
    mainWindow.webContents.send("update-background");
  }, 2000);
